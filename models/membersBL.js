...
async function deleteMovieFromSubArray(movieName, memberId) {
    ...
    let index = updatedMoviesArr.findIndex(movie => String(movie.movieName) === String(movieName))
    console.log(index);
...
async function updateMovieName(prevMovieName, currentMovieName) {
    let allMembers = await getAllMembers()
    allMembers.forEach(member => {
        let newMoviesArr = member.MoviesArr
        let index = member.MoviesArr.findIndex(movie => movie.movieName === prevMovieName)
        if (index !== -1) {
            newMoviesArr[index].movieName = currentMovieName
            Member.findByIdAndUpdate(String(member._id), {
                MoviesArr: newMoviesArr
            }, function (err) {
                if (err) {
                    console.log(err)
                } else {
                    console.log('Updated moviename for member subscription')
                }
            }
            )
        }
...
module.exports = {
    getAllMembers, saveAllMembersInDB, updateMember, addMember, deleteMember, getMemberById, deleteMovieFromSubArray
    , updateMovieName
}